...
- (OSStatus)renderLightsaberSound:(BOOL *)isSilence
    ...
    *isSilence = NO;
    ...
    // Wrap phases periodically to prevent accumulation of floating point errors
    // Use fmod for better precision than simple subtraction
    self.fundamentalPhase = fmod(self.fundamentalPhase, 2.0 * M_PI);
    self.harmonicPhase = fmod(self.harmonicPhase, 2.0 * M_PI);
    self.vibratoPhase = fmod(self.vibratoPhase, 2.0 * M_PI);
...
- (void)updateWithLidAngle:(double)lidAngle {
    // Input validation: clamp lid angle to expected range
    if (isnan(lidAngle) || isinf(lidAngle)) {
        NSLog(@"[FuturisticAudioEngine] Invalid lid angle received: %f", lidAngle);
        return;
    }
    
    // Clamp to reasonable bounds (allow some overshoot beyond normal range)
    lidAngle = fmax(-10.0, fmin(180.0, lidAngle));
...
- (void)rampToTargetParameters {
    ...
    double currentTime = CACurrentMediaTime();
    if (self.lastRampTime == 0.0) self.lastRampTime = currentTime;
